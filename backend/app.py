...
EXAMPLE_PRIVACY_POLICIES: Dict[str, Path] = {f"{pp.stem.split('.')[0]}": pp for pp in DATA_PRIVACY_POLICY_DIR.glob('*.md')}
...
def get_text_summary():
    ...
    if 'docType' not in args and 'docName' not in args:
        ...
        return make_response(f"Missing query parameters: {', '.join(missing_parameters)}", 400)
    doc_type = args['docType']
    doc_name = args['docName']
    if doc_type == 'pp':
        if doc_name in EXAMPLE_PRIVACY_POLICIES:
            app.logger.info(f'Loading privacy policy text {str(EXAMPLE_PRIVACY_POLICIES[doc_name])}')
            document = EXAMPLE_PRIVACY_POLICIES[doc_name].read_text(encoding='utf-8')
            return document
        else:
            return make_response(f'The specified document type Privacy Policy does not have file {doc_name} available.', 400)
